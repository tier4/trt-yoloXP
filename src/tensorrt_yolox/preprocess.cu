...
__global__ void argmax_gpu_kernel(
				      int N, unsigned char * dst, float * src, int dst_h, int dst_w, int src_c, int src_h, int src_w, int batch)
{
  // NHWC
  int index = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
...
void argmax_gpu(unsigned char * dst, float * src, int d_w, int d_h, int s_w, int s_h, int s_c, int batch, cudaStream_t stream)
{
  int N = d_w * d_h;  
  argmax_gpu_kernel<<<
    cuda_gridsize(N), block, 0, stream>>>(N, dst, src, d_h, d_w, s_c, s_h, s_w, batch);
}
